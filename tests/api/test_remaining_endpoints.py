...
def test_unknown_peer_group_returns_404(client):
    response = client.get("/api/v1/peers/UNKNOWN")
...
def test_market_cap_invalid_ticker(client):
    response = client.get("/api/v1/market-cap/INVALIDXYZ")
...
def test_portfolio_stats(client):
    ...
    assert set(data["statistics"]) == expected_statistics
...
def test_tcs_documents(client):
    ...
    assert all(
        "year" in row and "annual_report" in row and "is_url_valid" in row
        for row in data["documents"]
    )


def test_invalid_documents_ticker(client):
    response = client.get("/api/v1/companies/INVALIDXYZ/documents")
...
def test_screener_api_matches_engine(client):
    ...
    engine_df = run_screener(engine_filters)
    ...
    api_ids = [row["company_id"] for row in api_data["companies"]]

    engine_ids = engine_df["company_id"].astype(str).tolist()
