...
def _resolve_group(
    ...
    available = [row["peer_group_name"] for row in rows]

    matched = next(
        (value for value in available if value.lower() == cleaned.lower()),
        None,
    )
    ...
    if matched is None:
        raise HTTPException(
            status_code=404,
            detail={
                "message": (f"Unknown peer group: {group_name}"),
                "available_peer_groups": (available),
            },
        )
...
def get_peer_group(
    ...
    for row in percentile_rows:
        ...
        metric_map[company_id][row["metric"]] = {
            "value": row["value"],
    ...
    for member in members:
        ...
        for metric_name in METRICS:
            company_metrics[metric_name] = metric_map.get(
                company_id,
                {},
            ).get(
                metric_name,
    ...
    benchmark = next(
        (company for company in companies if company["is_benchmark"]),
        None,
    )
...
def compare_company_to_peers(
    ...
    with _connect() as conn:
        ...
        if company is None:
            raise HTTPException(
                status_code=404,
                detail=(f"Unknown company ticker: " f"{ticker}"),
            )
        ...
        if group_row is None:
            raise HTTPException(
                status_code=404,
                detail=(f"No peer group found for " f"{ticker}"),
            )
        ...
        if benchmark_row is None:
            raise HTTPException(
                status_code=404,
                detail=(f"No benchmark configured for " f"{group_name}"),
            )

        benchmark_id = benchmark_row["company_id"]
    ...
    for row in peer_rows:
        ...
        percentile_lookup[row["company_id"]][row["metric"]] = row["percentile_rank"]

    scores = _score_lookup()

    radar_metric_names = [metric_name for metric_name, _ in RADAR_METRICS]

    labels = {metric_name: label for metric_name, label in RADAR_METRICS}

    axes = []

    company_ids = sorted(percentile_lookup.keys())

    for metric_name in radar_metric_names:
        company_value = percentile_lookup.get(ticker, {}).get(metric_name)

        benchmark_value = percentile_lookup.get(benchmark_id, {}).get(metric_name)

        peer_values = [
            percentile_lookup[company_id].get(metric_name) for company_id in company_ids
        ]
    ...
    group_scores = [scores.get(company_id) for company_id in company_ids]

    group_scores = [value for value in group_scores if value is not None]
    ...
    axes.append(
        {
            ...
            "peer_average": (composite_peer_average),
            "benchmark": scores.get(benchmark_id),
        }
    )
    ...
    return {
        ...
        "peer_company_count": len(company_ids),
        "benchmark": {
            "company_id": benchmark_id,
            "company_name": (
                None if benchmark_company is None else benchmark_company["company_name"]
            ),
        },
        "axis_count": len(axes),
        "axes": axes,
