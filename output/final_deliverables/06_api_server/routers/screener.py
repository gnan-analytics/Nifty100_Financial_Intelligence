...
def get_screener(
    ...
    if min_rev_cagr_5yr is not None:
        filters["revenue_cagr_5yr"] = min_rev_cagr_5yr

    if min_pat_cagr_5yr is not None:
        filters["pat_cagr_5yr"] = min_pat_cagr_5yr
    ...
    if sector is not None:
        ...
        available_sectors = sorted(
            result["broad_sector"].dropna().astype(str).unique().tolist()
        )

        matched_sector = next(
            (value for value in available_sectors if value.lower() == sector.lower()),
            None,
        )
        ...
        result = result[
            result["broad_sector"].astype(str).str.lower() == matched_sector.lower()
